...
def train(cfg_param = None, using_gpus = None):
    os.makedirs(f'./output/{args.output}',exist_ok=True)
    #Train dataloader
    transforms = get_transformations(cfg_param, is_train = True)
    train_data = Yolodata(mode='train',
    ...
    for i in using_gpus:
        print(f"gpus {i} {torch.cuda.get_device_name(i)}")
        print(f"total memory : {get_memory_total_MiB(i)} free memory : {get_memory_free_MiB(i)}")
    ...
    if len(using_gpus) == 1:
        ...
        print("multi-gpu : {}".format(using_gpus))
        device = torch.device("cuda:"+str(using_gpus[0]) if torch.cuda.is_available() else "cpu")
        model = torch.nn.DataParallel(model, device_ids=using_gpus)
        model = model.cuda()
    ...
    torch_writer = SummaryWriter(f"./output/{args.output}")
