...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    ap, p, r = [], [], []
    prog_bar = tqdm(unique_classes, bar_format='{l_bar}{bar:20}{r_bar}')
    for c in prog_bar:
        i = pred_cls == c
        n_gt = (target_cls == c).sum()  # Number of ground truth objects
        n_p = i.sum()  # Number of predicted objects

        prog_bar.set_description(desc='Computing AP')
...
def drawBoxlist(_img, boxes : list = [], class_list:list=[], mode : int = 0, name : str = ""):
    _img = _img * 255
    #img dim is [C,H,W]
    if _img.shape[0] == 3:
        _img_data = np.array(np.transpose(_img, (1,2,0)), dtype=np.uint8)
    ...
    for box in boxes:     
        ...
        if mode == 0:
            ...
            draw.text((box[0],box[1]-17), str(round(float(box[4]),2)), fill =color, font=font)
            draw.text((box[0],box[1]-32), str(class_list[int(box[5])]), fill =color, font=font)
    #img_data.show("draw")
    img_data.save('./result/'+name+".png")
